...
const Paginator = ({ page, totalCounts, itemsPerPage, pageGroupSize, onChangePage} : PaginatorProps) => {
    ...
    return <PaginatorContainer>
        ...
        { pageNumbers?.map((number) => <PgNumber key={number} onClick={() => onChangePage(number)} $isCurrent={number === page}>{number + 1}</PgNumber>)}
        <PgBtnWrapper>
            <NextSVG onClick={()=> page === totalPages - 1  && onChangePage(page + 1)}/>
        </PgBtnWrapper>
    </PaginatorContainer>
}
